...
def test_generate_row() -> None:
    row = _generate_row(cells_count=6)
...
def test_reverse_block() -> None:
    block = _generate_block(7, 8)
    reversed_block = [list(reversed(i)) for i in block]
    tested_block = _reverse_block(block)
    assert tested_block == reversed_block, tested_block


def test_concatenate_blocks() -> None:
    rows_count = 3
    left_block = _generate_block(7, rows_count)
    spacer = _generate_block(1, rows_count)
    right_block = _reverse_block(left_block)
    tested_block = _concatenate_blocks(left_block, spacer, right_block)
